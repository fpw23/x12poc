...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    copy: {
      ...
      deployCode: {
        files: [
          { expand: true, cwd: './dist/', src: ['**'], dest: 'docker/web/code/' },
  ...
  // setup tasks
  grunt.registerTask('start', 'Start debugging in development mode', function () {
  ...
  grunt.registerTask('run', 'Start without debugging in production mode', function () {
    grunt.task.run('clear')
    grunt.task.run('init')
    grunt.task.run('check')
    grunt.task.run('build')

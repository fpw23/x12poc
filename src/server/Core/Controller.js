import { FunctionResult, FunctionResultStatus, MessageBuilder } from './FunctionResult'
...
export const TestSecurity = (requirements, auth, name) => {
  ...
  if (requirements.accountRequired === true) {
    if (!auth.account) {
      return MessageBuilder.Error(`You must be signed in to call the ${name} method`)
    }
  }
  ...
  if (requirements.userRequired === true) {
    if (!auth.user) {
      return MessageBuilder.Error(`You must be logged into an agency to call the ${name} method`)
    }
  }

  if (requirements.accountPermissions) {
    if (_.isArray(requirements.accountPermissions)) {
      if (auth.account.HasPermissions(requirements.accountPermissions, false) === false) {
        return MessageBuilder.Authorization(`You do not have the required account permissions to call the ${name} method`)
      }
    } else if (_.isString(requirements.accountPermissions)) {
      if (auth.account.HasPermission(requirements.accountPermissions) === false) {
        return MessageBuilder.Authorization(`You do not have the required account permissions to call the ${name} method.`)
      }
    } else {
  ...
  if (requirements.userPermissions) {
    if (_.isArray(requirements.userPermissions)) {
      if (auth.user.HasPermissions(requirements.userPermissions, false) === false) {
        return MessageBuilder.Authorization(`You do not have the required agency user permissions to call the ${name} method`)
      }
  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (_.isObject(config.inputModel) === false) {
        ret.AddMessageError(`Invalid controller config for method ${config.name}, must have an input model`)
        ret.Status = FunctionResultStatus.Failure
        res.json(ret)
        return
      }
